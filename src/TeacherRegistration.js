...
function Registration() {
    ...
    const { createTeacher, currentUser } = useAuth()
    ...
    async function handleRegister(e) {
        ...
        if(passwordRef.current.value.length < 5)
        {
            ...
            {
                await createTeacher(emailRef.current.value, passwordRef.current.value, nameRef.current.value,phonenumberRef.current.value,fileRef.current.files[0]).then((data)=>{
